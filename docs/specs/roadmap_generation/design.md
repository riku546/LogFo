...
ロードマップ機能は「AI生成」と「手入力」の2モードで構成します。

- **`RoadmapListPage`**: 一覧ページ。`新しいロードマップ` のプルダウンで `AI生成` / `手入力` を選択可能。
- **`RoadmapGeneratorPage`**: AI生成ページのコンテナ。状態管理とAPI呼び出しを統括。
...
- **`ManualRoadmapCreatePage`**: 手入力作成ページのコンテナ。基本情報入力とマイルストーン/タスク編集を提供。
  - **`hooks/useManualRoadmapCreate`**: 手入力モード専用のフォーム状態・編集状態・保存処理を管理。
  - **`components/ManualRoadmapEditor`**: 手入力モード専用の入力UI（マイルストーン/タスク CRUD）。
...
- **手入力作成状態 (Custom Hook):**
  `currentState` / `goalState` / `summary` と、マイルストーン/タスクのCRUD状態を保持し、保存時にAPIペイロードへ変換する。
...
- **備考:** 手入力モードも同一の保存APIを利用し、新規エンドポイントは追加しない。
...
4.  **手入力の保存前バリデーション:** 以下を満たさない場合は保存せず、フロントエンドでエラーメッセージを表示する。
    - `currentState` が空でない
    - `goalState` が空でない
    - マイルストーンが1件以上
    - タスクが全体で1件以上
